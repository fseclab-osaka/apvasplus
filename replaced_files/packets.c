...
bgp_encode_prefixes(struct bgp_proto *p, byte *w, struct bgp_bucket *buck, uint remains)
{
  ...
  sigListCounter = 0;
  while (!EMPTY_LIST(buck->prefixes) && (remains >= (5+sizeof(ip_addr)+80))){
      ...
      char m1[str_size] = "";
      bsprintf(m1, "ski1 65001 %I/%d", px->n.prefix, px->n.pxlen);
      //log("count: %d, m1: %s", sigListCounter, m1);
      ListAdd(list1, key1.pk, m1); 

      char m2[str_size] = "";
      bsprintf(m2, "ski2 65001 65002");
      //log("m2: %s", m2);
      ...
      //log("sigCounter: %d", sigListCounter);

      point_set(sign, sigList[sigListCounter++]);

      //my_point_print("signnnnnnnnnn", sign);
      
      SeqAggSign(sign, list1, &key2, m2);
      //my_point_print("sign", sign);
...
*/
      ...
      //char m1[5000] = "";
      char m1[str_size] = "";
      bsprintf(m1, "ski1 65001 %I/%d", prefix, pxlen);
      //log("m1: %s", m1);
      ListAdd(list1, key1.pk, m1); 

      Verify(sign_tmp, list1);
      //log("VERIFY2 %d", Verify(sign_tmp, list1));

      //署名リストに格納

      if (!ini_sigList){
        for (int takei = 0; takei < 10000; ++takei){
          point_init(sigList[takei], para.prg->g1);
        }
        ini_sigList = 1;
      }
      //log("count: %d", sigListCounter);
      point_set(sigList[sigListCounter++], sign_tmp);
      ...
      if(sigListCounter>maxcounter){
         maxcounter = sigListCounter;
         log("count: %d, max sign pool: %dKB", sigListCounter, (sigListCounter*64+64)/1000);
      }
